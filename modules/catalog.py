            ...
            for item in catalog_item:
                if input_search.lower() in item.title.lower():
                    if type(item) is Magazine:
                        ...
                        list_result.append(f'Title: {item.title}, DDS Number: {item.dds_number}, Type Catalog: Magazine')
                    elif type(item) is Dvd:
                        list_result.append(f'Title: {item.title}, actors: {item.actors}, Type Catalog: Magazine')
                    elif type(item) is Cd:
                        list_result.append(f'Title: {item.title}, artist: {item.artist}, Type Catalog: Magazine')                    
        return list_result
